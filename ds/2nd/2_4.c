...
float get_mean(student *classmates, int n) /* 리턴값의 타입을 실수형으로하고 배열 students의 포인터와 학생수 n을 매개변수로 넘겨받은 함수 */
{
...
double get_deviation(student *classmates, int n, float average) /* 리턴값의 타입을 실수형으로하고 배열 students의 포인터와 학생수 n, 실수형 성적의 평균값을 매개변수로 넘겨받은 함수 */
{
    double var, stdev;          /* 분산을 저장하는 var, 표준편차를 저장하는 stdev 선언 */
    double temp = 0;            /* 계산값을 임시로 저장하는 temp 선언하고 초기화 */
...
int main()
{
    ...
    double avg;                                                    /* 성적의 평균을 저장할 실수형 변수 avg를 선언 */
    double sd;                                                     /* 성적의 표준편차를 저장할 실수형 변수 sd를 선언 */
    ...
    student *classmate = students;                                 /* classmates = &students[0]; classmates에 students[0]번째 주소값을 저장 */
    avg = get_mean(classmate, num);                                /* 포인터 classmate와 학생 수를 넘겨줘서 get_mean 함수 호출하고 리턴값을 avg에 저장 */
    sd = get_deviation(classmate, num, avg);                       /* 포인터 classmate와 학생의 수, 구한 평균을 전달하여 성적의 표준편차를 구하는 함수를 호출하고 리턴값을 sd에 저장 */
    printf("average is = %f, standard deviation is %lf", avg, sd); /* 구한 성적의 평균값과 표준편차값을 출력하는 printf함수 */
}